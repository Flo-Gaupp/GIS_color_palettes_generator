...
const headline = document.getElementById("headline_userPalettes");
...
async function loadMoreRequestGET() {
    const response = await fetch(`${url}loadMoreUserPalettes`);
...
async function createHeadline() {
    ...
    dataPromise.then(async ()=> {
        if (await dataPromise !== "none signed in user") {
            headline.innerHTML = `Nice to see you ${await dataPromise}`;
        } else {
